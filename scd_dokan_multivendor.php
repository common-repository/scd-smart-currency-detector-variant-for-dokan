...
function scd_save_product_prices($post_id, $data) {
    ...
    if ($user_curr !== FALSE && $user_curr !== get_option('woocommerce_currency') && $curr_opt == 'only-default-currency') {
		...
		update_post_meta($post_id, '_meta_post_currency', $scd_currencyVal);
...
function scd_dokan_menu($menu) {
    ...
    $menu['scd_dokan_menu'] = array(
        'title' => __('SCM Currency', 'scd_dokan_marketplace'),
...
/*
// new other way to  add scd menu on dokan dashboard
...
add_filter( 'dokan_query_var_filter', 'register_scd_queryvar');
    ...
    function load_scd_templates( $template_path, $template, $args ) {
        ...
        if ( isset( $args['is_scd_currency'] ) && $args['is_scd_currency'] ) {
            ...
            return WP_PLUGIN_DIR . '/scd-smart-currency-detector-variant-for-dokan/templates';
        }
    ...
    if ($user_curr == FALSE || $user_curr_opt == 'selected-currencies') {
        ...
        if ($user_curr_opt == 'base-and-default-currency') {
            $contents = ' <div class="content-half-part">
                                <label for="scd_regular_price" class="dokan-form-label">' . __('SCM Price', 'scd_dokan_marketplace') . '</label>
                            ...
                            <div class="content-half-part sale-price">
                                ...
                                ' . __('SCM Discounted Price  ', 'scd_dokan_marketplace') . '</label>
                                <div class="dokan-input-group">
                                    <span class="dokan-input-group-addon">' . $curr_symbol . '</span>
            ...
            if ($product)
            $earning=dokan()->commission->calculate_commission($post->ID, floatval($saleprice) );
        } else { //base vurrency
            $contents = '';
        }
    }
 ...
 if($regprice){
    ...
    echo '<div class="dokan-form-group dokan-clearfix dokan-price-container">
            <div class="content-half-part">
                <div class="dokan-input-group">
                    <input required type="text" size="100" id="source_variable_regular_price'.$loop.'" name="scd_variable_regular_price[' . $loop . ']" value="' . $regprice . '" class="wc_input_price dokan-form-control dokan-product-regular-price-variable" placeholder="' . __('Variation price (required)', 'scd_dokan_marketplace') . '">
                </div>
            </div>';
     }else{
         echo '<div class="dokan-form-group dokan-clearfix dokan-price-container">
            <div class="content-half-part">
                <div class="dokan-input-group">
                    <input required type="text" size="100" id="source_variable_regular_price'.$loop.'" name="scd_variable_regular_price[' . $loop . ']" value="" class="wc_input_price dokan-form-control dokan-product-regular-price-variable" placeholder="' . __('Variation price (required)', 'scd_dokan_marketplace') . '">
                </div>
            </div>';
     }
    if($saleprice){
    ...
    echo '<input type="hidden" id="scd_dokan_wholesale_price_' . $loop . '" class="scd_dokan_wholesale_price" value="' . $wholesale_price . '" />';
     $_product= wc_get_product($variation->ID);
      if ($_product)
     $earning=dokan()->commission->calculate_commission( $variation->ID, floatval($saleprice) ); 
    ...
    <script type="text/javascript">
        ...
        jQuery('.variation-topbar-heading').click(function (e) {
            ...
            if (is_filled == 1) {
                ...
                var new_lab = 'Sale price (' + curr_symb + ') <a href="#" class="sale_schedule">Schedule</a><a href="#" class="cancel_sale_schedule" style="display:none">' . __('Cancel schedule', 'scd_dokan_marketplace') . '</a>';
                ...
                setTimeout(function () {
                   ...
                   jQuery('.vendor-earning').html( __('( You Earn : ', 'scd_dokan_marketplace') + curr_symb + '<span class="vendor-price">'+jQuery(vendor_earn).val()+'</span> )');
...
function scd_dokan_get_seller_earnings($earning, $seller_id) {
    ...
    if (!$installed_version || version_compare($installed_version, '2.8.2', '>')) {
        $debit = $wpdb->get_results($wpdb->prepare(
                    ...
                    WHERE
                        vendor_id = %d AND DATE(balance_date) <= %s AND status IN ($status) AND trn_type = 'dokan_orders'", $seller_id, $on_date));
        $debit_balance = 0;
        ...
        $credit = $wpdb->get_results($wpdb->prepare(
                    ...
                    FROM {$wpdb->prefix}dokan_vendor_balance
                    WHERE
                        vendor_id = %d AND DATE(balance_date) <= %s AND trn_type = %s AND status = %s", $seller_id, $on_date, $trn_type, $refund_status));
        ...
        $results = $wpdb->get_results($wpdb->prepare(
                    ...
                    WHERE
                        seller_id = %d AND DATE(p.post_date) <= %s AND order_status IN ($status)", $seller_id, $on_date));
        $total_earninigs = 0;
...
add_action('dokan_product_list_table_after_status_table_data','scd_dokan_product_list',10,4);

function scd_dokan_product_list($post, $product, $tr_class, $row_actions){
    ?>
    <style>#dokan-product-list-table{display:none;}</style>
    <script>
        jQuery(document).ready(function(){
           jQuery('[data-title="<?php esc_attr_e( 'SKU', 'dokan-lite' ); ?>"]').remove(); 
           jQuery('[data-title="<?php esc_attr_e( 'Stock', 'dokan-lite' ); ?>"]').remove(); 
           jQuery('[data-title="<?php esc_attr_e( 'Earning', 'dokan-lite' ); ?>"]').remove(); 
           jQuery('[data-title="<?php esc_attr_e( 'Price', 'dokan-lite' ); ?>"]').remove(); 
        });
    </script>
<td data-title="<?php esc_attr_e( 'scd_SKU', 'dokan-lite' ); ?>">
    <?php
    if ( $product->get_sku() ) {
    ...
    <td data-title="<?php esc_attr_e( 'scd_Stock', 'dokan-lite' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'scd_Price', 'dokan-lite' ); ?>">
        <?php
        $child = array();
        $price1 = "";
        $price2 = "";
        ...
        if ( $product->get_children() ) {
           // echo wp_kses_post( $product->get_price_html() );
            foreach($product->get_children() as $product_child){
				...
				if(get_post_meta($post->ID, 'scd_other_options', true)){
					
					if(get_post_meta($product_child, '_list_regular_price',true)){
						
					$simplePrice1 = get_post_meta($product_child, '_list_regular_price',true);
                    $simplePrice2 = get_post_meta($product_child, '_list_sale_price',true);	
					
					}else{
						
					$simplePrice1 =	scd_function_convert_subtotal(get_post_meta($product_child, '_regular_price',true), get_option('woocommerce_currency'), scd_get_user_currency(), 2);
                    $simplePrice2 = scd_function_convert_subtotal(get_post_meta($product_child, '_sale_price',true), get_option('woocommerce_currency'), scd_get_user_currency(), 2);   
				    ...
				    $simplePrice1 = scd_function_convert_subtotal(get_post_meta($product_child, '_regular_price', TRUE), get_option('woocommerce_currency'), scd_get_user_currency(), 2);
                    $simplePrice2 = scd_function_convert_subtotal(get_post_meta($product_child, '_regular_price', TRUE), get_option('woocommerce_currency'), scd_get_user_currency(), 2);
				}
				
            
                if(get_post_meta($product_child, '_sale_price')[0]!=""){
                    array_push($child,$simplePrice2);
                }else{
                    array_push($child,$simplePrice1);
                }
           }
           asort($child);
           $price1 = $child[array_key_first($child)];
           $price2 = $child[array_key_last($child)];
           echo get_woocommerce_currency_symbol(scd_get_user_currency()).''.$price1.' - '.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$price2;
			...
			if(get_post_meta($post->ID, 'scd_other_options', true)){
					...
					$simplePrice1 =	scd_function_convert_subtotal(get_post_meta($post->ID, '_regular_price',true), get_option('woocommerce_currency'), scd_get_user_currency(), 2);
                    $simplePrice2 = scd_function_convert_subtotal(get_post_meta($post->ID, '_sale_price',true), get_option('woocommerce_currency'), scd_get_user_currency(), 2);   
            ...
            if($simplePrice2){
                echo '<span style="text-decoration: line-through; color:red;">'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$simplePrice1.'</span> <span>'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$simplePrice2.'</span>';
            }else{
                echo '<span>'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$simplePrice1.'</span>';
            }
    ...
    <td data-title="<?php esc_attr_e( 'scd_Earning', 'dokan-lite' ); ?>">
        <?php
        if ( $product->get_type() == 'variable' ) {
            $price = dokan_get_variable_product_earning( $product->get_id() );
            echo $price;
            /*$price = scd_function_convert_subtotal($price, get_option('woocommerce_currency'), scd_get_user_currency(), 2);
            echo wp_kses( $price, $price_kses );*/
            //  echo get_woocommerce_currency_symbol(scd_get_user_currency()).''.$price1.' - '.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$price2;
        } else {
			if ($simplePrice2 == "0" || $simplePrice2 == ""){
            $price = dokan()->commission->calculate_commission($post->ID, floatval($simplePrice1)  );
			}else {
					$price = dokan()->commission->calculate_commission($post->ID, floatval($simplePrice2)  );
				  }
							
            echo '<span>'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$price.'</span>';
            /*$price = scd_function_convert_subtotal($price, get_option('woocommerce_currency'), scd_get_user_currency(), 2);
            echo wp_kses( $price, $price_kses );*/
            /*if($simplePrice2){
              echo '<span>'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$simplePrice2.'</span>';
            }else{
              echo '<span>'.get_woocommerce_currency_symbol(scd_get_user_currency()).''.$simplePrice1.'</span>';
            }*/
     ...
     <script>
        jQuery(window).load(function() {
           jQuery('#dokan-product-list-table').css('display','block');
        });
    </script>
    <?php
}
...
add_filter('woocommerce_review_order_before_payment', 'choice_currency',999,2);
function choice_currency(){ //echo scd_get_target_currency();
	$method_pay = [
	'USD' => 'Dolares $',
	'BOB' => 'Bolivianos Bs.'
];
...
<span class="title">Pay in: </span>
	<?php foreach ($method_pay as $key => $value) : ?>
<div>
   <input type="radio" name="currency_payment" id="payment_method_<?php echo $key?>" class="payment_method_<?php echo $key?>" value="<?php echo $key ?>" />

	<label class="label_scd" for="payment_method_<?php echo $key ?>" style="font-size: 25px;"><?php echo $value ?></label> 
	<style>
    ...
    border-radius:50%;
    outline:none;
    box-shadow:0 0 5px 0px #f49917 inset;
}
...
//quick view dokan edit update


add_filter( 'dokan_get_template_part', function($template, $slug, $name ){
    
	add_filter('scd_enable_js_conversion',function($enableJsConversions){

        return FALSE;
    });
    
    if($slug == 'products/edit/product-list-table-inline-edit-form'){
        return SCDS_PLUGIN_DIR_PATH .'templates/product-list-table-inline-edit-form.php';
    }
    return $template;
},999,3);

add_filter( 'dokan_update_product_post_data', 'scd_dokan_update_product_post_data',10,1 );

function scd_dokan_update_product_post_data($data){
	...
	$priceField = 'regular_' . $user_curr . '_' . $data['_regular_price'] . '-sale_' . $user_curr . '_' . $data['_sale_price'];
	
    update_post_meta($product_id, 'scd_other_options', array(
            "currencyUserID" => $scd_userID,
            "currencyUserRole" => $scd_userRole,
            "currencyVal" => $user_curr,
            "currencyPrice" => $priceField
     ));
	
	update_post_meta($product_id, '_quick_regular_price', $data["_regular_price"]);

	update_post_meta($product_id, '_quick_sale_price', $data["_sale_price"]);
	
	
	
	
	
	$data["_regular_price"] = scd_function_convert_subtotal($data["_regular_price"], $user_curr, get_option('woocommerce_currency'),  2); 
	
	$data["_sale_price"] = scd_function_convert_subtotal($data["_sale_price"], $user_curr, get_option('woocommerce_currency'),  2);
...
/*
add_action( 'wp_ajax_dokan_product_inline_edit',function(){
    $post_data = wp_unslash( $_POST );
	$product_id = $post_data['data']['ID'];
	
    $_regular_price=$post_data['data']["_regular_price"];
// 	$_regular_price=get_post_meta($post_data['data']['ID'], '_list_regular_price',true);
    $_sale_price=$post_data['data']["_sale_price"];
// 	$_sale_price=get_post_meta($product_id, '_list_sale_price',true);
    $user_curr = scd_get_user_currency();
// 	$_regular_price =100; //get_post_meta($product_id, '_list_regular_price', $data['regular_price']);
// 	update_post_meta($product_id, '_list_regular_price', $_regular_price);
// 	update_post_meta($product_id, '_list_sale_price', $_sale_price);
	$_regular_price = scd_function_convert_subtotal($_regular_price, $user_curr, get_option('woocommerce_currency'), 2);
	$_sale_price = scd_function_convert_subtotal($_sale_price, $user_curr, get_option('woocommerce_currency'), 2);
//     update_post_meta($product_id, '_list_regular_price', scd_function_convert_subtotal($_regular_price, get_option('woocommerce_currency'), $user_curr, 2));
//     update_post_meta($product_id, '_list_sale_price', scd_function_convert_subtotal($_sale_price, get_option('woocommerce_currency'), $user_curr, 2));
//     $post_data['data']["_regular_price"] = get_post_meta($product_id, '_list_regular_price',true);
    $post_data['data']["_regular_price"] = $_regular_price;
    $post_data['data']["_sale_price"] = $_sale_price;
//     $_POST['data'] = $post_data['data'];
},1,0 );

// add_filter( 'dokan_update_product_post_data', function($data){
	
// 	$data["_regular_price"]= '100';
// 	$data["_sale_price"]= '50';
...
//   add_action( 'dokan_product_list_table_after_row', 'scd_dokan_product_list_table_after_row', 999, 2 );
// function scd_dokan_product_list_table_after_row(){
