<?php
/*
  Plugin Name: SCM - Smart Currency Manager -  Premium Variant for Dokan
  ...
  Text Domain: scd_dokan_marketplace
 */
...
add_action('init', function () {
    if ( is_admin() && in_array('scd-smart-currency-detector-variant-for-dokan/index.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
...
function scd_lazone_load_dokan_domaine() {
    load_plugin_textdomain( 'scd_dokan_marketplace', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}
add_action( 'plugins_loaded', 'scd_lazone_load_dokan_domaine' );
...
function scd_multi_add_scrypt_topost() {
        wp_enqueue_script("scd-dokan-multivendor", trailingslashit(plugins_url("", __FILE__)) . "js/scd_dokan_multivendor.js", array("jquery"));
        wp_enqueue_script("scd-pro_ready", trailingslashit(plugins_url("", __FILE__)) . "js/scd_pro_postready.js", array("jquery"));
		$variable_to_js = [
    		'ajax_url' => admin_url('admin-ajax.php')
    	];
    	wp_localize_script('scd-dokan-multivendor', 'scd_ajax', $variable_to_js);
...
function scd_license_settings($tabs) {
    $lic_manager =array('id'=>'license','label'=> __('LICENSE MANAGER', 'scd_dokan_marketplace'),'class'=>'nav-tab','page'=>'scd_options_page','name'=>'scd_license_options','submit'=>false);
...
function scd_license_manager() {
            ...
            ?>
                <div class="scd-notice" style="margin-top: 10px">
                    <p> <?php echo  __('Your SCM License is not activated or has expired. Please activate a new license to get access to the plugin features and support.', 'scd_dokan_marketplace'); ?> </p>
                    <p> <?php echo __('You can get new license key', 'scd_dokan_marketplace'); ?>  <a href="https://gajelabs.com/" target="_blank"> <?php echo __('here', 'scd_dokan_marketplace'); ?> </a></p>
                </div>
            <?php
            } 
...
<div>
                <hr />
                <input type="submit" id="scd_activate_new" class="button-primary scd_save" value=" <?php echo __('Activate New License', 'scd_dokan_marketplace'); ?> " />
                <hr />
            </div>
              <?php
}
...
register_activation_hook(__FILE__, 'scd_dokan_plugin_install');

function scd_dokan_plugin_install(){
    if(!get_option('scd_dokan_first_install_date')){
        update_option('scd_dokan_first_install_date',new DateTime('now'));
    }   
}


function scd_dokan_features_date(){
	...
	if(get_option('scd_dokan_first_install_date')){
		...
		if(scds_check_license_active()){
             ...
             $fist_install_date = get_option('scd_dokan_first_install_date');
             $reste = 0;
             $text = "";
			 ...
			 if($reste > 6*24*60 && $reste <= 7*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 7 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . "<a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . " </span>"  ; 
            }else if($reste > 5*24*60 && $reste <= 6*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 6 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . "</span>"  ;  
            }else if($reste > 4*24*60 && $reste == 5*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 5 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . " </span>"  ;  
            }else if($reste > 3*24*60 && $reste <= 4*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 4 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . "</span>"  ;  
            }else if($reste > 2*24*60 && $reste <= 3*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 3 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . " </span>"  ;  
            }else if($reste > 1*24*60 && $reste <= 2*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 2 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a> " . __(' to activate or update your license key.', 'scd_dokan_marketplace') . "</span>"  ;  
            }else if( $reste > 0 && $reste <= 1*24*60){
				$text = "<span style='color:black;'>" . __('You have less than 1 days to enjoy for free SCM Premium for Dokan. Please activate your license to enjoy and to get automatic updates or support! Click', 'scd_dokan_marketplace') . "<a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . " </span>"  ; 
            }else if($reste < 0){
				$text = "<span style='color:black;'>" . __('The SCM-Multivendors for Dokan license key has not been activated or has expired, so you will be unable to get automatic updates or support! Click', 'scd_dokan_marketplace') . " <a href='". admin_url('admin.php?page=scd_options_page&tab=license') ."' >" . __('here', 'scd_dokan_marketplace') . "</a>" . __(' to activate or update your license key.', 'scd_dokan_marketplace') . " </span>"  ; 
			}
		}
	}
...
function scd_license_activation() {
    ...
    if (isset($_REQUEST['activate_license'])) {
        ...
        if (is_wp_error($response)) {
            echo "<h3 style='color: red;'>" . __('Sorry the server could not activate your license key. Try again later or contact us.', 'scd_dokan_marketplace') . "</h3>";
        } else if (strpos($response['body'], 'Error 404') !== FALSE) {
            echo "<h3 style='color: red;'>" . __('ERROR 404. The activation server was not found. Please contact contact us to correct.', 'scd_dokan_marketplace') . "</h3>";
        }
        ...
        if ($license_data->result == 'success') {//Success was returned for the license activation
            ...
            echo '<br /><h3 style = "color : red;" > ' . $license_data->message . __( '. Please wait the page will reload in ', 'scd_dokan_marketplace') . ' <span id="abc">3</span>  ' . __( '. seconds...', 'scd_dokan_marketplace') . '</h3>'
                    . '<script>' 
                    . 'var count = 2;
...
if(!is_plugin_active('scd-smart-currency-detector/index.php')){
    echo '<h3 style="color:red;">' . __('SCM Multivendors PRO require scm-smart-currency-manager before use, please ', 'scd_dokan_marketplace') . ' <a target="__blank" href="https://wordpress.org/plugins/scd-smart-currency-detector/">' . __(' download and install it here', 'scd_dokan_marketplace') . '</a><h3>';    
}
}
...
if (scds_check_license_active()){
    require 'scd_dokan_multivendor.php';
} else{
  ...
  if(is_admin())
    {
        ...
        try
        {
           include 'scd_update_checker_dokan.php';
        }
        catch(\Error $e)
        {
            echo ( __('Smart Currency Detector (SCM) plugin error: ', 'scd_dokan_marketplace') . $e->__toString());
        }
        catch(\Exception $e)
        {
            echo ( __('Smart Currency Detector (SCM) plugin exception: ', 'scd_dokan_marketplace')  . $e->__toString());    
        }
    }    
